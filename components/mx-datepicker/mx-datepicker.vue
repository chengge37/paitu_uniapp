<template>
	<view v-if="isShow" class="picker" @click="onCancel">
		<!-- 日期选择器 -->
		<view v-if="type!='time'" class="picker-modal" @click.stop @touchend.stop>
			<view class="picker-modal-header">
				<view :class="increYear?'picker-icon-grey':''" class="picker-icon picker-icon-zuozuo" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetYear('-1')"></view>
				<view :class="reduseMonth?'picker-icon-grey':''" class="picker-icon picker-icon-zuo" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetMonth('-1')"></view>
				<text class="picker-modal-header-title">{{title}}</text>
				<view :class="increMonth?'picker-icon-grey':''"  class="picker-icon picker-icon-you" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetMonth('+1')"></view>
				<view :class="increYear?'':'picker-icon-grey'" class="picker-icon picker-icon-youyou" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetYear('+1')"></view>
			</view>
			...
			<view class="picker-modal-footer"  v-if="showDate">
		...
		<!-- 时间选择器 -->
		<view v-if="showTimePicker" class="picker" @click.stop @touchend.stop>
...
<script>
	...
	const DateTools = {
		...
		isPastDate(d) {
			let date = new Date();
			return d.getMonth()<date.getMonth() || (d.getMonth()==date.getMonth()&&d.getDate()<date.getDate())
		},
		/**
		 * 格式化Date对象
		 * @param d 日期对象
		...
		getCalendar(date, proc) {
			...
			for (let i = 0; i < 42; i++) {
				let tmp = {
					...
					isPastDate: DateTools.isPastDate(it)
				};
		...
		formatTimeArray(t, s) {
			...
			r.forEach((v, k) => {
				...
				r[k] = (('0' + JSON.stringify(v)).slice(-2))
			});
	...
	export default {
		...
		methods: {
			...
			setValue(value) {
				this.date = new Date();
				this.currentYear = this.selectYear = this.date.getFullYear();
				this.currentMonth = this.selectMonth = this.date.getMonth()
				this.checkeds = [];
				...
				if (value) {
					...
					// let has_busy = false;
					// let dateAdd = this.util.deepcopy(this.date);
					// dateAdd.setDate(this.date.getDate()+1)
					// this.busyDate.forEach(v => {
					// 	if(DateTools.isSameDay(this.util.strToDate(v),new Date(this.date))){
					// 		has_busy = true
					// 	}
					// })
					// has_busy?this.checkeds.push(new Date(dateAdd)):this.checkeds.push(new Date(this.date));
				}
			...
			//改变年份
			onSetYear(value) {
				if(this.currentYear==this.date.getFullYear()){
					if(value>0){
			...
			//改变月份
			onSetMonth(value) {
				if(this.currentYear==this.date.getFullYear()&&this.currentMonth>=this.date.getMonth()){
					if(value>0){
			...
			procCalendar(item) {
				...
				item.statusStyle = {
					opacity: 1,
					color: item.isOtherMonth || item.isPastDate ? '#ddd' : '#000',
				...
				// 不可用日期
				if(this.busyDate.length>0){
					this.busyDate.forEach(v => {
						if(DateTools.isSameDay(this.util.strToDate(v),item.dateObj)){
							item.statusStyle.opacity = 0.3
						}
					})
				}
				...
				if (this.checkeds.length == 2) {
					...
					if (!item.bgStyle.type && (+item.dateObj > +this.checkeds[0] && +item.dateObj < +this.checkeds[1])) { //中间的日期
						...
						if(this.busyDate.length>0){
							...
							if(has_busy){
								this.checkeds = []
								uni.showToast({title:'被占用日期不可选!',icon:'none'});
			...
			onSelectDate(date) {
				if(this.selectable){
					...
					let has_busy = false;
					this.busyDate.forEach(v => {
						if(DateTools.isSameDay(new Date(date.dateObj), this.util.strToDate(v))){
							has_busy = true;
						}
					});
					if(has_busy){
						uni.showToast({title:'该日期已被占用!',icon:'none'})
					}else{
			...
			onConfirm() {
				...
				if (this.type == 'time') {
					...
					let time = DateTools.formatTimeArray(this.timeValue,this.showSeconds)
		...
		computed: {
			...
			increYear(){
				return this.currentYear==this.selectYear?true:false
			},
			reduseMonth(){
				return this.currentYear==this.selectYear&&this.currentMonth==this.selectMonth?true:false
			},
			increMonth(){
				return this.currentYear+1==this.selectYear&&this.selectMonth==12?true:false
			}
		},
		watch: {
