...
class Socket {
	...
	SocketEvents({
		...
		return new Promise((resolve, reject) => {
			...
			uni.onSocketOpen(res => {
				console.error('uni.onSocketOpen被调用');
				this.CONCATCOUNT += 1;
			...
			uni.onSocketClose(async err => {
				console.log('uni.onSocketClose被调用');
			...
			uni.onSocketError(err => {
				console.error('uni.onSocketError 被调用 ' +JSON.stringify(err));
				SocketTask.isconnect = false;
				resolve(false);
				if (!this.isRconnectIng) {
					this.hbDetection();
	...
	initChat({
		...
		console.log('initChat被调用');
		this._chunkConnect(() => {
			console.log('关闭in inChat',this.SocketTask);
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				console.log('init Chat res = '+JSON.stringify(res))
				resolve(res);
			} catch (e) {
				console.log(e)
				reject();
	...
	connectSocket(url, args) {
		return new Promise((resolve, reject) => {
			//console.log('connectSocket'+JSON.stringify(args) + url,);
			uni.connectSocket({
				url:this.SocketTask.url,
				success: () => {
					console.log('connectSocket  succ');
					resolve();
				},
				fail: (err) => {
